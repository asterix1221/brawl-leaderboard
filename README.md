...
### Миграции и расширение pgcrypto
- Миграции используют функцию `gen_random_uuid()` и требуют расширения `pgcrypto`. Первая миграция (`001_create_users_table.sql`) теперь содержит `CREATE EXTENSION IF NOT EXISTS pgcrypto;`, а скрипты `migrate.sh`/`migrate.bat` дополнительно устанавливают расширение перед применением файлов, чтобы не зависеть от порядка запуска.
- Для CI/локальных проверок запустите PostgreSQL через `docker-compose up -d postgres`, затем выполните `./tests/Integration/migrations_smoke.sh` — скрипт применит миграции и проверит наличие `pgcrypto`, падая с ошибкой при любом сбое.
